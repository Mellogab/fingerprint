...
const FingerprintJS = (function (exports) {
    ...
    ***************************************************************************** */
     ...
     */         
    function getFlashVersion (options)
    {
        const keys = []
        const strOut = null;
        ...
        try
        {
            ...
            {
                const objPlayerVersion = window.swfobject.getFlashPlayerVersion();
                const strVersion = objPlayerVersion.major + '.' + objPlayerVersion.minor + '.' + objPlayerVersion.release;
                if (strVersion === '0.0.0') strVersion = 'N/A';
        ...
        if (!options.excludeFlashVersion) keys.push({ 'flash_version': strOut || 'N/A' });
     ...
     */      
    function getFontSmoothing (options)
    {
        const keys = []
        ...
        else
        {
            try
            {
                ...
                for (let j = 8; j <= 32; j = j + 1)
                {
                    for (let i = 1; i <= 32; i = i + 1)
                    {
                        let imageData = ctx.getImageData(i, j, 1, 1).data;
        ...
        if (!options.excludeFontSmoothing) keys.push({ 'font_smoothing': strOut });
     ...
     */     
    function getJavaEnabled(options)
    {
        const keys = [];
        let strOut = false;
        ...
        try
        {
            if (typeof navigator.javaEnabled !== 'undefined') strOut = navigator.javaEnabled();
        } catch (err)
        {
            strOut = 'Error';
        }

        if (!options.excludeJava) keys.push({ 'java_enabled': strOut });
     ...
     */    
    function getMicrosoftSilverlightVersion(options)
    {
        debugger;
        const keys = [];
        let strOut = null;
        ...
        try
        {
            ...
            {
                let objPlugin = navigator.plugins['Silverlight Plug-In'];
        ...
        if (!options.excludeMicrosoftSilverlightVersion)
            keys.push({ 'ms_silverlight_version': strOut || 'Error' });
        
        return keys;
    }

    /**
     *  Get the coordinates based on HTML5 API
     */      
    function getCoordinates()
    {
        ...
        if (navigator.geolocation){
            navigator.geolocation.getCurrentPosition((position) => 
                keys.push({ 'coordinates': position.coords || 'Error' })
            );
        }
    ...
    /**
     *  Get the ip server based on api rest
     */     
    function getServerIP(){
        const xmlHttp = new XMLHttpRequest();
        xmlHttp.open('GET',enviroments.ip_api,false); // false for synchronous request
        xmlHttp.send(null);
    ...
    const sources = {
        ...
        /**
         * Basicamente o hash do fingerprint é gerado beasedo nesse objeto sources.
         * Logo se qualquer valor desse for alterado, será produzido um novo hash, é caso do resolução da tela
         * , mudando a resolução podemos obter dois hash diferentes para a mesma máquina , para o mesmo browser
         * , porém em abas diferentes
         */        
    ...
    // changed source with some more properties of professional version
    const sourcesFullOptions = {
        // custom methods
        microsoftSilverlightVersion: getMicrosoftSilverlightVersion,
        javaEnabled: getJavaEnabled,
        fontSmoothing: getFontSmoothing,
    ...
    function getBuiltinComponentsAdditional() {
        return getComponents(sourcesFullOptions, options, []);
    }
     ...
     */
    function makeLazyGetResult(components, additional) {
        let visitorIdCache;
    ...
    class OpenAgent {
        ...
        get() {
            return __awaiter(this, void 0, void 0, function* () {
                const components = yield getBuiltinComponents(options);
                const additionalComponents = yield getBuiltinComponentsAdditional(options);
                const result = makeLazyGetResult(components, additionalComponents);
